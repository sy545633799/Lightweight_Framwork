...
local module_updater = require("Hotfix/module_updater")
local functions_replacer = require("Hotfix/functions_replacer")
...
function M.hotfix_module(file_name)
    assert("string" == type(file_name))
    local module_name = string.gsub(file_name, '/', '.')
    if not package.loaded[module_name] then
        M.log_debug("Skip unloaded module: " .. module_name)
        return package.loaded[module_name]
    end
    M.log_debug("Hot fix module: " .. file_name)

    --local file_path = assert(package.searchpath(file_name, package.path))
    local fp = assert(io.open(Util.LuaPath .. '/' .. file_name .. '.lua', "rb"))
    local chunk = fp:read("*all")
    fp:close()

    -- Load chunk.
    local func = assert(loadstring(chunk))
    local ok, obj = assert(pcall(func))
    if nil == obj then obj = true end  -- obj may be false

    hotfix_module_with_obj(module_name, obj)

    print(string.format("hotfix:%s", file_name))
    return package.loaded[module_name]
end
