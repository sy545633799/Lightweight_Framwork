...
/// 该触发器根据相机裁剪区域触发
/// </summary>
public class SceneCameraDetector : DetectorBase
{
	...
	public override bool IsDetected(Bounds bounds, out bool isInView)
	{
		if (m_Camera == null)
		{
			isInView = false;
		...
		isInView = m_Camera.IsBoundsInCamera(bounds);
		return m_Camera.IsBoundsInCamera(bounds, -horizontalExtDis, horizontalExtDis, -bottomExtDis, topExtDis);
	}
	...
	{
		if (m_Camera)
			GizmosEx.DrawViewFrustumEx(m_Camera, -horizontalExtDis, horizontalExtDis, -bottomExtDis, topExtDis, Color.yellow);
	}
#endif
}
