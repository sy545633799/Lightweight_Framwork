...
namespace Game
{
    public class Logger
    {
        ...
        {
#if UNITY_EDITOR
            Debug.Log((p != null && p.Length > 0 ? string.Format(s, p) : s));
#else
        ...
        private static void SendToHttpSvr(string url, string postData)
        {
            if (!string.IsNullOrEmpty(postData))
            {
                ...
                m_webClient.UploadStringAsync(new Uri(url), "POST", postData);
            }
        }
