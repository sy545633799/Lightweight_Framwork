...
{
    public class DefineSpritePacker : UnityEditor.Sprites.IPackerPolicy
    {
        ...
        public void OnGroupAtlases(BuildTarget target, UnityEditor.Sprites.PackerJob job, int[] textureImporterInstanceIDs)
        {
            ...
            foreach (int instanceID in textureImporterInstanceIDs)
            {
                ...
                if (target == BuildTarget.Android)
                    PlatformSettings = ti.GetPlatformTextureSettings("Android");
                else if (target == BuildTarget.iOS)
                    PlatformSettings = ti.GetPlatformTextureSettings("iPhone");
                else
                    PlatformSettings = ti.GetDefaultPlatformTextureSettings();

                Texture2D textureFile = AssetDatabase.LoadAssetAtPath(ti.assetPath, typeof(Texture2D)) as Texture2D;
                ...
                foreach (Sprite sprite in sprites)
                {
                    ...
                    entry.sprite = sprite;
                    bool IsUseHighQuality = (desiredFormat != TextureFormat.ETC_RGB4 && desiredFormat != TextureFormat.ETC2_RGBA8);
                    if (target == BuildTarget.Android)
                        entry.settings.format = IsUseHighQuality ? TextureFormat.ASTC_RGBA_6x6 : TextureFormat.ETC_RGB4;
                    else if (target == BuildTarget.iOS)
                        entry.settings.format = ti.DoesSourceTextureHaveAlpha() ? TextureFormat.ASTC_RGBA_6x6 : TextureFormat.ASTC_RGBA_6x6;
                    else
                    ...
                    entry.settings.filterMode = Enum.IsDefined(typeof(FilterMode), ti.filterMode)
                        ? ti.filterMode
                        : FilterMode.Bilinear;
                    entry.settings.maxWidth = textureFile.width > kDefaultMaxSprite ? kDefaultMaxSprite * 2 : kDefaultMaxSprite;
                    entry.settings.maxHeight = textureFile.height > kDefaultMaxSprite ? kDefaultMaxSprite * 2 : kDefaultMaxSprite;

                    if (PlatformSettings.maxTextureSize > kDefaultMaxSprite && textureFile.width > kDefaultMaxSprite || textureFile.height > kDefaultMaxSprite)
                        Debug.LogWarningFormat("UI贴图【{0}】大小超过1024-》请不要将其打包成图集", ti.assetPath);
                    ...
                    if (target == BuildTarget.Android)
                        entry.settings.allowsAlphaSplitting = IsUseHighQuality ? false : ti.DoesSourceTextureHaveAlpha();
                ...
                Resources.UnloadAsset(textureFile);
            }
